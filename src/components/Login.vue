...
<!--      登陆头像-->
      <div class="img_box">
        <img src="../assets/logo.png" alt="loge图片">
      </div>
<!--      登录表单-->
      <el-form ref="loginFromRef" :model="loginFrom" :rules="loginFormRules" label-width="0px" class="login_form" >
  ...
  methods: {
    ...
    login () {
      this.$refs.loginFromRef.validate(async valid => {
        if (!valid) return
        const { data: ref } = await this.$http.post('login', this.loginFrom)
        if (ref.meta.status !== 200) return this.$message.error('用户名或密码错误')
